...
tldr: 强化学习技能适配在动态杂乱环境中常因无约束探索导致不安全与不稳定。本文提出Dist-GPRL，基于高斯过程参数化技能，顺序适配稀疏轨迹途经点的局部重叠窗口，并利用HAP安全子空间先验、动态距离场奖励与轨迹运动学相似正则。仿真与真机实验表明任务成功率更高、碰撞更少、学习更稳定，且保留示教运动学特征。
source: arxiv
...
motivation: 动态杂乱环境中的强化学习技能适配需安全探索，固定观测或严格探索计划等假设限制稳定性，无约束探索易导致碰撞与学习不稳。
method: Dist-GPRL基于高斯过程参数化技能，顺序适配稀疏轨迹途经点的局部重叠窗口；结合HAP安全子空间先验、动态距离场间隙奖励和轨迹运动学相似正则。
result: 仿真动态物体操作与真机迁移实验中，Dist-GPRL任务成功率高于基线，碰撞频率更低，学习更稳定，并保留示教技能的速度与加速度特征。
conclusion: 该框架为动态环境中的安全技能适配提供了可行方案，通过距离感知与安全引导提升学习稳定性，同时维持示教轨迹的运动学一致性。
---

## 摘要
基于强化学习的技能自适应框架通常需要限制性假设来保持稳定性，例如固定观测或严格控制的探索调度。然而，在杂乱且动态的环境中，无约束探索可能导致不安全行为和不稳定学习，尤其是当任务相关观测位于障碍物附近或涉及移动物体时。在这项工作中，我们提出了 Dist-GPRL，一种面向结构化机器人技能自适应的距离感知与安全引导强化学习框架。该框架基于高斯过程（GP）的技能参数化，顺序自适应稀疏轨迹途经点的重叠局部窗口，而不是在每个策略步骤修改完整技能。原始策略输出通过 GP 协方差结构进行关联，产生时间上连贯的轨迹更新，同时减少与全局轨迹自适应相关的动作空间和信用分配困难。安全性通过两种互补形式的引导纳入其中。由豪斯多夫近似规划器（HAP）导出的安全子空间先验将策略探索偏向可行区域，而动态更新的距离场净空与梯度奖励提供局部障碍物感知。轨迹-运动学相似性正则化器进一步在自适应过程中保持示教的速度和加速度特性。我们在仿真中的两个动态物体操作任务上评估该框架，并将学习到的策略迁移到真实机器人执行中。实验结果表明，与基线相比，该框架具有更高的任务成功率、更低的碰撞频率和更稳定的学习，同时保持示教技能的运动学特性。
