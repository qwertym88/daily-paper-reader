...
tldr: 针对自主机器人行为规划需兼顾反应性、可解释性和适应性的需求，现有方法多基于行为树（BT），而有限状态机（FSM）虽广泛高效却缺乏生成式方法。本文提出生成式部分指定有限状态机（GPSFSM）神经符号架构，通过Fabric引擎解析执行含顺序、恢复、并行等控制结构的行为计划，结合异步事件系统和参数注入实现标准化语义描述及LLM动态生成。导航实验表明，GPSFSM在计划生成成功率上持续优于前沿BTGenBot系统，尤其在零样本场景表现突出，延迟与前沿LLM相当。该工作首次将生成式FSM规划用于机器人，开源ROS2栈提升了实用性与可复现性。
source: arxiv
...
motivation: LLM与行为树结合取得进展，但FSM因其符号结构和计算效率尚未被生成式方法探索，限制了复杂行为规划的可解释性与适应性。
method: 提出GPSFSM神经符号架构，包括Fabric引擎（解析执行含多种控制结构）、Capabilities2扩展（异步事件与参数注入）及PromptTools（统一LLM接口与提示缓冲），实现基于FSM的生成式行为规划。
result: 导航任务中，GPSFSM计划生成成功率高于BTGenBot，零样本场景优势显著，且规划延迟与前沿LLM持平或更低。
conclusion: 首次实现生成式FSM规划，开源ROS2栈使机器人行为规划更实用、可复现且可解释。
---

## 摘要
自主机器人在动态环境中运行需要结合反应性、可解释性和适应性的行为规划系统。虽然大语言模型已成功与行为树集成用于动态重规划，但有限状态机尽管被广泛采用且计算高效，却仍未在生成式方法中得到探索。我们提出一种生成式部分指定有限状态机（GPSFSM）神经符号架构，利用FSM的符号和语义结构（包括状态和事件触发转换）来实现行为规划。本文介绍了首个用于机器人的GPSFSM框架，包含Fabric——一个FSM引擎，用于解析、验证和执行包含顺序、恢复、并行任意和并行全部控制结构的行为计划。我们在ROS2中扩展了Capabilities2包，增加了异步事件系统用于行为链式执行和运行时参数注入以实现可配置执行，解决了限制当前生成系统的临时函数表示问题。PromptTools为本地和云端LLM提供统一的ROS2接口，支持提示缓冲，实现任务和上下文信息的动态异步组合。这些组件共同实现了与机器人无关的标准语义能力描述。在导航任务上的实验评估表明，我们的GPSFSM方法在计划生成成功率上持续优于最先进的BTGenBot系统，尤其是在零样本场景中（BT通常表现不佳），同时保持与前沿LLM相当或更低的规划延迟。我们还展示了系统能够生成复杂行为。我们发布了一个开源的ROS2栈，使得生成式FSM规划对机器人系统实用且可复现。
